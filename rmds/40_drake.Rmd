...
One large challenge for literate programing is that for every small change in a document you need to re-run your complete analysis. This is not a problem on modern computers in many cases, but when your data is large enough, it can become a hassle. While RMarkdown does provide a caching mechanism, it is limited to the individual computer and may not be shared among researchers. And when individual code chunks depend on external data that has changed, RMarkdown caching no longer registers these changes. 

The packge `r cite_pkg("drake")` addresses this challenge. By creating a plan using the `drake_plan` function we first determine what steps are necessary for our analysis. Drake then analyzes our code and files for implicit dependencies. It derives a depency tree (see Fig. \@ref(fig:drake)) that visually shows how the project should run.
...
```
Each build target (e.g., `report`) is the call to an R function. The result of which, can easily be reloaded anywhere inside the project using the `loadd` or `readd` function. In the simplistic build path for this document the `hist` target creates a histogram of the `iris_data`, which is downloaded from the OSF. The figure (see Fig. \@ref(fig:drakeout)) is than included in the document. Whenever a single previous dependency becomes outdated, the rest of the dependency graph is executed.


```{r drakeout, out.width="0.6\\linewidth", include=TRUE, fig.align="center", fig.cap=c("This figure was created  outside of the document."), echo = FALSE}
library(drake)
readd("hist")
```

When all dependencies are properly modelled, drake allows running individual targets on multiple CPUs or a compute cluster without much overhead. The interested read can take a look at the `make.R` file in the project root of this document. 

## Ensuring Relative Paths Using `here`
The `r cite_pkg("here")` provides a useful tool to find files on all operating systems. Windows and Unix are famous for using different slashes as directory separators, which can make addressing file paths complicated for reproducible research. By encapsulating all file operations in the `here` function, the relative root is set to the destination of the R project file and directory separators are automatically inserted in accordance with the local operating system.
...
1. Create project using `create_tidy_project`
...
The following plot was created from a single line of code.
